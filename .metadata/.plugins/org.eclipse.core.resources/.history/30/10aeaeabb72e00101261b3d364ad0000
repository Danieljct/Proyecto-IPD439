...
int32_t mg_x_values[NUM_MUESTRAS]; // Sufficient size
int32_t mg_y_values[NUM_MUESTRAS];
int32_t mg_z_values[NUM_MUESTRAS];
...
static void Start_FIFO_Read_DMA(void)
{
    ...
    // Buffer RX: Usaremos dma_rx_buffer (global)
    // Limpiar buffer RX (opcional, buena práctica)
    // memset(dma_rx_buffer, 0, bytes_to_read + 1);
    ...
    // Medir tiempo de INICIO DMA
    time = TIM2->CNT;

    // CS BAJO (Manual, antes de llamar a HAL DMA)
    GPIOA->BSRR = GPIO_PIN_1<< 16;

    // Iniciar DMA SPI Tx/Rx
    ...
    difftime = TIM2->CNT - time; // Tiempo que tardó en INICIAR el DMA

    if (dma_status != HAL_OK) {
        // Error al iniciar DMA
...
static void Process_FIFO_Data_DMA(uint16_t bytes_received_incl_dummy)
{
    ...
    for (int i = 0; i < sets_leidos; i++) {
            ...
            mg_x_values[i] = (int32_t)((float)raw_x_values[i] * cached_acc_sensitivity);
            mg_y_values[i] = (int32_t)((float)raw_y_values[i] * cached_acc_sensitivity);
            mg_z_values[i] = (int32_t)((float)raw_z_values[i] * cached_acc_sensitivity);

            /*
            // --- Optional: Print for first/last few samples (like in user's snippet) ---
            if (i < 5 || i >= (sets_leidos - 5)) {
                printf(" Set[%3u]: Ax = %5ld mg, Ay = %5ld mg, Az = %5ld mg\r\n",
